...
"""
=====================================================================
   Phase field Unified Formulation - AT1-AT2 model
       >> Fenics implementaion code :
           o Dynamic simulation
           o Implemented with miehe History Variable
=====================================================================
...
LS_Rtol       = 1e-7 # relative tolerance for Linear solver (PF eq.)
LS_Atol       = 1e-9 # absoulte tolerance for Linear solver (PF eq.)
LS_maxiter    = 1000   # max. iteration for Linear solver (PF eq.)
...
sigc = (3/8/1.95e-4*Gc*E)**(0.5)   # Contrainte critique (ou limite elastique) [Pa]
...
Res_folder = './Results/'+base_file+'_Miehe'+Model+'_'+current_datetime+'/'    # Dossier resultats
...
if Behav == 'Sym':
    ...
    def H(u, Hold, Model):
        # Calculate Psi using the Psi0 function with Behav as an argument
        Psi = Psi0(u, Behav)
        
        # Calculate H1 based on Psi and Hold
        H1 = conditional(gt(Psi, Hold), Psi, Hold)
        
        # Select Model for further calculations
        if Model == 'AT1': 
            H0 = 3*Gc/16/l
        ...
        # Calculate Hist based on H1 and H0
        Hist = conditional(lt(H1, H0), H0, H1)
...
if Behav == 'DevDecompHyb':
    ...
    def H(u, Hold, Model):
        ...
        # Calculate H1 based on Psi and Hold
        H1 = conditional(gt(PsiP, Hold), PsiP, Hold)
...
def alpha(d, Model):
    ...
    - Model (str): Model type, e.g., 'AT1' or 'AT2'.
...
# Damage and history fields from previous and new step 
phi, pold, Hold = Function(Vdam), Function(Vdam), Function(Vdam)
...
# Damage problem
EDam_0 = ( (H(u, Hold, Model) * g_p(dphi) * q) + Gc/c0/l * ( alpha_p(dphi, Model)* q + 2*l*l* dot( grad(dphi), grad(q) ) ) )  * dx 
for bc in bc_dam:
    bc.apply(phi.vector())
# Define the linear variational problems
DamProblem  = LinearVariationalProblem(lhs(EDam_0), rhs(EDam_0),  phi, bc_dam)
# Construct solvers for displacement and phase field
DamSolver = LinearVariationalSolver(DamProblem)
# Damage Solver Parameters
DamSolver.parameters.update({
    "krylov_solver": {
        "absolute_tolerance": LS_Atol,
        "relative_tolerance": LS_Rtol,
        "maximum_iterations": LS_maxiter,
...
pold.assign(phi)
Hold.assign(project(H(u_new, Hold, Model), VHist))
...
EDam = ( (H(u_new, Hold, Model) * g_p(dphi) * q) + Gc/c0/l * ( alpha_p(dphi, Model)* q + 2*l*l* dot( grad(dphi), grad(q) ) ) )  * dx 

# Define the linear variational problems
DamProblem  = LinearVariationalProblem(lhs(EDam), rhs(EDam),  phi, bc_dam)
...
while float(t) < Tf:
    ...
    # updating the Hold to account for the irreversibility
    Hold.assign(project(H(u_new, Hold, Model), VHist))
