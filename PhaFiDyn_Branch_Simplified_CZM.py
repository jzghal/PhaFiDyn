...
# Damage problem
EDam_0 = ((g_p(phi) * (H(u, Hold, sigc, E, Model)  * q)
        +
        (Gc / c0 / l * (alpha_p(phi, Model) * q + 2 * l * l * dot(grad(q), grad(phi))))) * dx)
J_EDam_0 = ((g_pp(phi) * H(u, Hold, sigc, E, Model)  * q * dphi)
          +
...
DamSolver.parameters.update({"nonlinear_solver": "snes",
                        "snes_solver": {
                                        "method": "newtonls", 
                                        "line_search": "cp", 
...
pold.assign(phi)
Hold.assign(project(H(u, Hold, sigc, E, Model), VHist))
...
EDam = ((g_p(phi) * (H(u_new, Hold, sigc, E, Model)  * q)
        +
        (Gc / c0 / l * (alpha_p(phi, Model) * q + 2 * l * l * dot(grad(q), grad(phi))))) * dx)

J_EDam = ((g_pp(phi) * H(u_new, Hold, sigc, E, Model)  * q * dphi)
          +
          (Gc / c0 / l * (alpha_pp(phi, Model) * q * dphi + 2 * l * l * dot(grad(q), grad(dphi))))) * dx
...
while float(t) < T:
    ...
    print('------------------------------------------------------------- ')
    print('------------------------------------------------------------- ')
    print("Increment: %2.8g, Time: %2.8g" % (Incr, float(t)))
    print('------------------------------------------------------------- ')
    print('------------------------------------------------------------- ')
    ...
    # Update Hold to account for irreversibility
    Hold.assign(project(H(u_new, Hold, sigc, E, Model), VHist))
